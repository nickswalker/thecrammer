	...
	public static function get_dir_file_info($source_dir, $top_level_only = TRUE, $_recursion = FALSE)
	{
		...
		if ($fp = @opendir($source_dir))
		{
			...
			while (FALSE !== ($file = readdir($fp)))
			{
				...
				elseif (strncmp($file, '.', 1) !== 0)
				{
					$_filedata[$file] = self::get_file_info($source_dir.$file, array('settitle','setdescription','name','date'));
	...
	public static function get_file_info($file, $returned_values = array('name', 'server_path', 'size', 'date'))
	{
		...
		foreach ($returned_values as $key)
		{
			...
			case 'date':
				...
				break;
			case 'settitle':
				$fileinfo['title'] = (string)$file_xml->details->title;
				break;
			case 'setdescription':
				$fileinfo['description'] = (string)$file_xml->details->description;
				break;
			case 'readable':
